...
export class WalletService {
  ...
  async getOperatorRequests(limit = 50) {
    return this.prisma.transaction.findMany({
      ...
      select: {
        id: true, type: true, status: true, amountKobo: true, currency: true, reference: true,
        idempotencyKey: true, journalEntryId: true, completedAt: true, createdAt: true,
        user: { select: { id: true, email: true, firstName: true, lastName: true } },
      },
    });
  }

  forbiddenOperator(): never {
    throw new ForbiddenException('Only an active admin or compliance user can access wallet approvals');
  }
  ...
  async createDepositRequest(userId: string, dto: CreateWalletRequestDto) {
    ...
    if (existing) {
      if (existing.userId !== userId || existing.type !== TransactionType.DEPOSIT) throw new BadRequestException('idempotencyKey is already in use');
      return existing;
    }
    return this.prisma.transaction.create({ data: { userId, type: TransactionType.DEPOSIT, status: TransactionStatus.PENDING, amountKobo, currency, reference, idempotencyKey: dto.idempotencyKey, metadata: { workflow: 'customer_deposit' } } });
  }
  ...
  async createWithdrawalRequest(userId: string, dto: CreateWalletRequestDto) {
    ...
    const existing = await this.prisma.transaction.findUnique({ where: { idempotencyKey: dto.idempotencyKey } });
    if (existing) {
      if (existing.userId !== userId || existing.type !== TransactionType.WITHDRAWAL) throw new BadRequestException('idempotencyKey is already in use');
      return existing;
    }
    return this.prisma.transaction.create({ data: { userId, type: TransactionType.WITHDRAWAL, status: TransactionStatus.PENDING, amountKobo, currency, reference, idempotencyKey: dto.idempotencyKey, metadata: { workflow: 'customer_withdrawal' } } });
  }

  async confirmRequest(transactionId: string, adminUserId: string) {
    const admin = await this.prisma.user.findUnique({ where: { id: adminUserId } });
    if (!admin || !admin.isActive || !['ADMIN', 'COMPLIANCE'].includes(admin.role)) throw new ForbiddenException('Only an active admin or compliance user can confirm wallet transactions');
    return this.prisma.$transaction(async (tx) => {
      ...
      if (transaction.type === TransactionType.WITHDRAWAL) {
        const lines = await tx.journalLine.findMany({ where: { accountId: CUSTOMER_DEPOSITS_ACCOUNT_ID, metadata: { path: ['investorId'], equals: transaction.userId }, journalEntry: { status: EntryStatus.POSTED } }, select: { direction: true, amountKobo: true } });
        let availableKobo = 0n;
        for (const line of lines) availableKobo += line.direction === 'CREDIT' ? line.amountKobo : -line.amountKobo;
        if (availableKobo < transaction.amountKobo) throw new BadRequestException('Insufficient available balance');
      }
