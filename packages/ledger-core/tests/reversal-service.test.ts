/**
 * tests/reversal-service.test.ts
 *
 * Acceptance tests:
 * - Reversals net to zero
 * - Already-reversed entries are rejected
 * - Non-existent entries are rejected
 * - Non-POSTED entries are rejected
...
describe("ReversalService.reverse", () => {
  ...
  it("reversal nets balance to zero", async () => {
    ...
    // After reversal: both accounts net to 0
  ...
  it("reversal entry has mirrored lines", async () => {
    ...
    const result = await reversalSvc.reverse(
      { originalEntryId: original.id, idempotencyKey: "mirror-rev" },
      repo
    );
    ...
    if (result.ok) {
      ...
      expect(rev.reversalOfId).toBe(original.id);
      expect(rev.status).toBe("POSTED");

      // Each line in reversal should have opposite direction
      for (const origLine of original.lines) {
        const revLine = rev.lines.find(
          (l) => l.accountId === origLine.accountId
        );
  ...
  it("marks original entry as reversed", async () => {
    const original = await postDeposit(engine, repo, "mark-orig", 1_000n);

    const result = await reversalSvc.reverse(
      { originalEntryId: original.id, idempotencyKey: "mark-rev" },
      repo
    );
    expect(result.ok).toBe(true);

    const updated = await repo.findEntryById(original.id);
    expect(updated?.reversedById).toBeDefined();
    if (result.ok) {
      expect(updated?.reversedById).toBe(result.value.id);
    }
  });

  it("rejects reversal of non-existent entry", async () => {
    const result = await reversalSvc.reverse(
      { originalEntryId: "does-not-exist", idempotencyKey: "no-entry-rev" },
      repo
    );
  ...
  it("rejects reversal of a non-POSTED (REVERSED) entry", async () => {
    // Manually save a REVERSED-status entry
    const { InMemoryRepository: Repo } = await import("../src/repository.js");
    ...
    // Save with REVERSED status to simulate an already-reversed state
    const reversedEntry = { ...buildResult.value, status: "REVERSED" as const };
    await localRepo.saveEntry(reversedEntry);

    const result = await reversalSvc.reverse(
      { originalEntryId: reversedEntry.id, idempotencyKey: "rev-of-reversed" },
      localRepo
    );
  ...
  it("rejects double-reversal", async () => {
    ...
    await reversalSvc.reverse(
      { originalEntryId: original.id, idempotencyKey: "double-rev-1" },
      repo
    );

    const second = await reversalSvc.reverse(
      { originalEntryId: original.id, idempotencyKey: "double-rev-2" },
      repo
    );
  ...
  it("idempotency: same reversal key returns same entry", async () => {
    ...
    const first = await reversalSvc.reverse(
      { originalEntryId: original.id, idempotencyKey: "idem-rev-key" },
      repo
    );
    const second = await reversalSvc.reverse(
      { originalEntryId: original.id, idempotencyKey: "idem-rev-key" },
