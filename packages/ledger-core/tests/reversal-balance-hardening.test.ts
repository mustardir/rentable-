...
describe("balance derivation and reversal invariants", () => {
  ...
  it("ignores a non-POSTED entry even when its lines contain reversal metadata", () => {
    ...
    const pending: JournalEntry = {
      id: "pending-reversal",
      idempotencyKey: "pending-reversal-key",
      status: "PENDING",
      reversalOfId: "some-original",
      lines: [{
        id: "pending-line",
        journalEntryId: "pending-reversal",
        accountId: "acct_1100",
        direction: "DEBIT",
        amountKobo: 900_000n as any,
        metadata: {},
        createdAt: new Date(),
      }],
      postedAt: new Date(),
  ...
  it("does not double-count a reversal when calculating the investor balance", async () => {
    const engine = new PostingEngine();
    const repo = new InMemoryRepository();
    const reversalService = new ReversalService();
    const balanceService = new BalanceService();
    ...
    const firstReversal = await reversalService.reverse({ originalEntryId: original.id, idempotencyKey: "investor-reversal-command" }, repo);
