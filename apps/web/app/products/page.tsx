...
const products = [
  { code: "FORT-INVEST-001", name: "Fortress Investment", type: "Investment", currency: "USD", minimum: "$50.00", status: "Available", description: "A structured investment product designed for disciplined long-term capital growth." },
  { code: "FORT-FIXED-001", name: "Fortress Fixed Income", type: "Fixed Income", currency: "NGN", minimum: "₦50,000", status: "Coming soon", description: "A fixed-income product for investors seeking predictable income characteristics." },
];
          ...
          {products.map((product) => (
            <article key={product.code} className="rounded-3xl border border-white/10 bg-white/[0.06] p-7 shadow-2xl">
              <div className="flex items-start justify-between gap-4">
                <div><p className="text-xs font-medium tracking-widest text-slate-400">{product.code}</p><h2 className="mt-2 text-2xl font-semibold">{product.name}</h2></div>
              ...
              <Link href={`/products/${product.code}`} className="mt-6 block w-full rounded-xl bg-cyan-300 px-5 py-3 text-center font-semibold text-slate-950 transition hover:bg-cyan-200">View product</Link>
            </article>
          ))}
        </div>
        <p className="mt-10 max-w-3xl text-sm leading-6 text-slate-500">Investment products are subject to applicable terms, eligibility requirements and risk disclosures. This catalogue currently uses presentation data only; it does not access investor accounts or ledger balances.</p>
      </section>
      <SiteFooter />
    </main>
  );
